...
public class ContractListTreePart extends TreePart {

	private double sum;

	public ContractListTreePart(List<Contract> list, Action action) {
		super(list, action);
		setRememberColWidths(true);
//		this.addSelectionListener((event) -> {
//			if (event.data instanceof Contract) {
//				return;
//			}
//			try {
//				((Contract) event.data).setDoNotRemind(!"\u2611".equals(newValue));
//				((Contract) event.data).store();
//			} catch (RemoteException e) {
//				Logger.error("error while storing contract", e);
//				GUI.getStatusBar().setErrorText(Settings.i18n().tr("Error while storing contract"));
//			}
//		});
	}
...
//		return Settings.i18n().tr("Total in this month") + ": "
//				+ Settings.DECIMALFORMAT.format(Math.round(sum * 100d) / 100d) + " EUR";
//	}
}
